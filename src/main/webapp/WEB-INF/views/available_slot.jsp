...
<body class="flex min-h-screen">
    ...
    <main class="flex-1 p-10 overflow-y-auto">
        ...
        <!-- Recommended Tutors -->
        <section class="mb-8 bg-brand-surface rounded-[2rem] border border-white/5 p-6 shadow-2xl">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="preferredLanguage"
                                ...
                                <option value="Indonesia" ${user.preferredLanguage == 'Indonesia' ? 'selected' : ''}>🇮🇩 Indonesia</option>
                                <option value="Japanese"  ${user.preferredLanguage == 'Japanese'  ? 'selected' : ''}>🇯🇵 Japanese</option>
                                <option value="Korean"    ${user.preferredLanguage == 'Korean'    ? 'selected' : ''}>🇰🇷 Korean</option>
                                <option value="Chinese"   ${user.preferredLanguage == 'Chinese'   ? 'selected' : ''}>🇨🇳 Chinese</option>
                                <option value="French"    ${user.preferredLanguage == 'French'    ? 'selected' : ''}>🇫🇷 French</option>
                                <option value="German"    ${user.preferredLanguage == 'German'    ? 'selected' : ''}>🇩🇪 German</option>
                                <option value="Spanish"   ${user.preferredLanguage == 'Spanish'   ? 'selected' : ''}>🇪🇸 Spanish</option>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-4 flex items-center text-text-gray">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <c:forEach var="tutor" items="${recommendedTutors}">
                    <c:set var="recSlots" value="${tutorSlotsMap[tutor.id]}" />
                    <c:forEach var="slotRec" items="${recSlots}">
                        <div class="relative bg-[#1f1f1f] rounded-3xl p-5 border border-white/10 hover:border-brand-orange/50 transition-all duration-300 hover:shadow-[0_0_30px_rgba(249,115,22,0.1)] flex flex-col gap-3">
                            
                            <!-- Header -->
                            <div class="flex items-start justify-between">
                                <div>
                                    <div class="font-extrabold text-white text-sm">${tutor.name}</div>
                                    <div class="text-text-gray text-[10px] mt-0.5">${tutor.certification}</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-1.5">
                                <span><c:choose>
                                    <c:when test="${tutor.language == 'English'}">🇬🇧</c:when>
                                    <c:when test="${tutor.language == 'Indonesia'}">🇮🇩</c:when>
                                    <c:when test="${tutor.language == 'Japanese'}">🇯🇵</c:when>
                                    <c:when test="${tutor.language == 'Korean'}">🇰🇷</c:when>
                                    <c:when test="${tutor.language == 'Chinese'}">🇨🇳</c:when>
                                    <c:when test="${tutor.language == 'French'}">🇫🇷</c:when>
                                    <c:when test="${tutor.language == 'German'}">🇩🇪</c:when>
                                    <c:when test="${tutor.language == 'Spanish'}">🇪🇸</c:when>
                                    <c:otherwise>🌍</c:otherwise>
                                </c:choose></span>
                                <span class="text-white text-xs font-semibold">${tutor.language}</span>
                                <span class="text-text-gray text-[10px]">· ${tutor.yearsExperience} yrs exp</span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-1.5 text-[11px]">
                                <div class="text-text-gray">📅 ${slotRec.date}</div>
                                <div class="text-white font-medium text-right">${slotRec.startTime} - ${slotRec.endTime}</div>
                                <div class="text-text-gray">🎯 Level</div>
                                <div class="text-right"><span class="bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase">${slotRec.level}</span></div>
                                <div class="text-text-gray">📖 Lesson</div>
                                <div class="text-white font-medium text-right">${slotRec.lessonType}</div>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="text-brand-orange font-extrabold text-sm">
                                    IDR <fmt:formatNumber value="${slotRec.price}" type="number" groupingUsed="true" />
                                </div>
                                <a href="<c:url value='/bookings/create/${slotRec.id}'/>">
                                    <button class="bg-brand-orange hover:bg-orange-600 transition text-white font-bold text-[11px] px-4 py-2 rounded-xl">
                                        Book Now
                                    </button>
                                </a>
                            </div>
                ...
                <c:if test="${empty recommendedTutors}">
                    <div class="col-span-3 text-center text-text-gray text-sm py-10">
                                ...
                                <td>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-lg">
                                            ...
                                            <c:choose>
                                                <c:when test="${slot.tutor.language == 'English'}">🇺🇸</c:when>
                                                <c:when test="${slot.tutor.language == 'Indonesia'}">🇮🇩</c:when>
                                                <c:when test="${slot.tutor.language == 'Japanese'}">🇯🇵</c:when>
                                                <c:when test="${slot.tutor.language == 'Korean'}">🇰🇷</c:when>
                                                <c:when test="${slot.tutor.language == 'Chinese'}">🇨🇳</c:when>
                                                <c:when test="${slot.tutor.language == 'French'}">🇫🇷</c:when>
                                                <c:when test="${slot.tutor.language == 'German'}">🇩🇪</c:when>
                                                <c:when test="${slot.tutor.language == 'Spanish'}">🇪🇸</c:when>
                                                <c:otherwise>🌍</c:otherwise>
                                            </c:choose>
